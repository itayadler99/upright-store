...
import { offer, orderBump } from "@/lib/content";
...
export default function OfferCard() {
  ...
  const handleCheckout = () => {
    clear();
    add({
      id: subscribe ? "kit-sub" : "kit-once",
      name: subscribe
        ? "UPRIGHT Kit — Subscribe & Save"
        : "UPRIGHT Kit — One-time",
      price: kitPrice,
      qty: 1,
      recurring: subscribe,
    });
    if (bump) {
      add({
        id: "bump-minerals",
        name: "Extra month of minerals (30 sticks)",
    ...
    <div className="rounded-3xl bg-oat p-6 shadow-xl ring-1 ring-line md:p-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-clay">
          ...
          <span className="ml-1 text-xs text-muted">4.8 · 4,000+ kits</span>
        </div>
      </div>
